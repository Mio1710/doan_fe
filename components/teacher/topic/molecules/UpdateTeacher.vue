...
const updateTeacher = () => {
  $api.teacher.updateTeacher(form.value.id, form.value).then(() => {
...
<template>
  <form-card can-cancel cancel-text="Hủy" title="Cập nhật giảng viên" @cancel="emit('cancel')" @submit="updateTeacher">
    ...
    <app-text-field v-model="form.ten" name="Tên giảng viên" rules="required" />
    <app-text-field v-model="form.maso" name="Mã số" />
    <div class="d-flex">
