  ...
  >
    ...
    <div class="order-details">
      ...
      <div class="d-flex justify-between m-2">
        <v-btn color="success" @click="onAllowOrder">Duyệt</v-btn>
        <v-btn color="error" @click="onDenyOrder">Từ chối</v-btn>
      </div>
    </div>
  </form-approve-order-dialog>
</template>
...
import FormApproveOrderDialog from '@/components/common/organisms/FormApproveOrderDialog'

export default defineComponent({
  name: 'DetailOrderDialog',
  components: { FormApproveOrderDialog },
  ...
  setup() {
    ...
    const onAllowOrder = async () => {
      loading.value = true
      await $api.order
        .updateOrder(order.value.id, {
          order_status: 2
        })
        .then((res) => {
          order.value = res.data
          $dialog.notify.success('Cập nhật trạng thái thành công')
        })
        .finally(() => {
