...
const createStudentTopic = () => {
  $api.admin.createStudentTopic(form.value).then(() => {
    $toast.success('Thêm mới sinh viên khóa luận thành công')
...
<template>
  <form-card can-cancel cancel-text="Hủy" title="Thêm mới sinh viên khóa luận" @cancel="emit('cancel')" @submit="createStudentTopic">
