...
const importStudentIntern = () => {
  $api.studentIntern.importStudentIntern(serialize({ file: file.value })).then(() => {
    $toast.success('Import sinh viên thực tập thành công')
    emit('cancel')
  })
}
...
<template>
  <form-card
    can-cancel
    cancel-text="Hủy"
    title="Import sinh viên thực tập"
    @cancel="emit('cancel')"
    @submit="importStudentIntern"
  >
    <div class="mt-5">
      Tải xuống mẫu import dữ liệu:
      <a download href="/files/student_intern_template.xlsx" target="_blank">download</a>
    </div>
    <div class="mt-4">
...
const importStudentIntern = () => {
  $api.studentIntern
    .importStudentIntern(serialize({ file: file.value }))
    .then((res) => {
      if (!res.status) {
