...
const createStudentIntern = () => {
  $api.admin.createStudentIntern(form.value).then(() => {
    $toast.success('Thêm mới sinh viên thực tập thành công')
...
<template>
  <form-card can-cancel cancel-text="Hủy" title="Thêm mới sinh viên thực tập" @cancel="emit('cancel')" @submit="createStudentIntern">
